(function (global, factory) {
  ...
  var gsap,
    ...
    _getProxyProp = function _getProxyProp(
      element,
      property
    ) {
      return (
    ...
    _getScrollFunc = function _getScrollFunc(
      element,
      _ref
    ) {
      var s = _ref.s,
    ...
    _getSizeFunc = function _getSizeFunc(
      scroller,
      isViewport,
      _ref2
    ) {
    ...
    _getOffsetsFunc = function _getOffsetsFunc(
      element,
      isViewport
    ) {
    ...
    _iterateAutoRefresh = function _iterateAutoRefresh(
      func,
      events
    ) {
    ...
    _getComputedStyle = function _getComputedStyle(
      element
    ) {
      return _win.getComputedStyle(element);
    },
    _makePositionable = function _makePositionable(
      element
    ) {
    ...
    _getLabelRatioArray = function _getLabelRatioArray(
      timeline
    ) {
      var a = [],
    ...
    _getClosestLabel = function _getClosestLabel(
      animation
    ) {
    ...
    _getLabelAtDirection = function _getLabelAtDirection(
      timeline
    ) {
    ...
    _swapPinIn = function _swapPinIn(
      pin,
      spacer,
      cs,
      spacerState
      ...
      if (pin.parentNode !== spacer) {
        ...
        spacerStyle[_width] =
          _getSize(pin, _horizontal) + _px;
        spacerStyle[_height] =
          _getSize(pin, _vertical) + _px;
        spacerStyle[_padding] = pinStyle[
          _margin
        ] = pinStyle[_top] = pinStyle[_left] = "0";
    ...
    _setState = function _setState(state) {
      if (state) {
        var style = state.t.style,
          ...
          value;
        (
          state.t._gsap || gsap.core.getCache(state.t)
        ).uncache = 1;
    ...
    _getTweenCreator = function _getTweenCreator(
      scroller,
      direction
    ) {
  ...
  var ScrollTrigger = (function () {
    ...
    _proto.init = function init(vars, animation) {
        ...
        scrubSmooth &&
          (scrubTween = gsap.to(animation, {
            ...
            onComplete: function onComplete() {
              return (
                onScrubComplete && onScrubComplete(self)
              );
            },
          }));
        snap1 = 0;
      ...
      if (snap) {
        ...
        "scrollBehavior" in _body.style &&
          gsap.set(
            isViewport ? [_body, _docEl] : scroller,
            {
              scrollBehavior: "auto",
            }
          );
      ...
      if (pin) {
        ...
        if (!pinCache.spacer) {
          pinCache.spacer = spacer = _doc.createElement(
            "div"
          );
          spacer.setAttribute(
            "class",
          ...
          pinCache.pinState = pinOriginalState = _getState(
            pin
          );
        } else {
      ...
      if (markers) {
        ...
        offset =
          markerStartTrigger["offset" + direction.op.d2];
      ...
      self.refresh = function (soft, force) {
        ...
        var size = getScrollerSize(),
          ...
          triggerIndex =
            (trigger &&
              Math.max(0, _triggers.indexOf(self))) ||
            0,
          i = triggerIndex,
          cs,
          bounds,
        ...
        if (pin) {
          ...
          if (useFixedPosition) {
            ...
            override[_margin] = override[
              _margin + _Top
            ] = override[_margin + _Right] = override[
              _margin + _Bottom
            ] = override[_margin + _Left] = "0";
      ...
      if (!_coreInitted) {
        ...
        if (gsap) {
          ...
          if (_body) {
            ...
            _horizontal.m =
              Math.round(bounds.left + _horizontal.sc()) ||
              0;
            border
            ...
            _transformProp = gsap.utils.checkPrefix(
              "transform"
            );
    ...
    ScrollTrigger.config = function config(vars) {
      ...
      if ("autoRefreshEvents" in vars) {
        ...
        _ignoreResize =
          (vars.autoRefreshEvents + "").indexOf(
            "resize"
          ) === -1;
      }
    };

    ScrollTrigger.scrollerProxy = function scrollerProxy(
      target,
      vars
    ) {
      ...
      for (p in vars) {
        ...
        if (p === "all") {
          ...
          if (mq) {
            ...
            if (~i) {
              _media[i + 1] = _combineFunc(
                _media[i + 1],
                func
              );
              _media[i + 2] = _combineFunc(
                _media[i + 2],
                result
              );
            } else {
    ...
    ScrollTrigger.clearMatchMedia = function clearMatchMedia(
      query
    ) {
  ...
  ScrollTrigger.getScrollFunc = function (
    element,
    horizontal
  ) {
  ...
  ScrollTrigger.addEventListener = function (
    type,
    callback
  ) {
