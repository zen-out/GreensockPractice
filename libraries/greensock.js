(function (global, factory) {
  ...
  var _config = {
    ...
    _install = function _install(scope) {
      return (
        (_installScope = _merge(scope, _globals)) && gsap
      );
    },
    ...
    _harness = function _harness(targets) {
      ...
      if (!(harnessPlugin = (target._gsap || {}).harness)) {
        ...
        while (
          i-- &&
          !_harnessPlugins[i].targetTest(target)
        ) {}
    ...
    _getCache = function _getCache(target) {
      return (
        target._gsap || _harness(toArray(target))[0]._gsap
      );
    },
    ...
    _forEachName = function _forEachName(names, func) {
      return (
        (names = names.split(",")).forEach(func) || names
      );
    },
    ...
    _parseVars = function _parseVars(params, type, parent) {
      ...
      if (type) {
        ...
        while (parent && !("immediateRender" in irVars)) {
          ...
          parent =
            _isNotFalse(parent.vars.inherit) &&
            parent.parent;
        }
    ...
    _numericIfPossible = function _numericIfPossible(
      value
    ) {
    ...
    _copyExcluding = function _copyExcluding(
      obj,
      excluding
    ) {
      var copy = {},
    ...
    _recacheAncestors = function _recacheAncestors(
      animation
    ) {
    ...
    _alignPlayhead = function _alignPlayhead(
      animation,
      totalTime
    ) {
    ...
    _postAddChecks = function _postAddChecks(
      timeline,
      child
    ) {
      var t;
      ...
      if (child._time || (child._initted && !child._dur)) {
        ...
        if (
          !child._dur ||
          _clamp(0, child.totalDuration(), t) -
            child._tTime >
            _tinyNum
        ) {
          child.render(t, true);
    ...
    _scrollTrigger = function _scrollTrigger(
      animation,
      trigger
    ) {
      return (
    ...
    _renderZeroDurationTween = function _renderZeroDurationTween(
      ...
      if (
        ...
        tween._zTime =
          totalTime || (suppressEvents ? _tinyNum : 0);
        suppressEvents ||
    ...
    distribute = function distribute(v) {
      ...
      return function (i, target, a) {
        ...
        if (!distances) {
          ...
          distances.u =
            getUnit(vars.amount || vars.each) || 0;
          ease = ease && l < 0 ? _invertEase(ease) : ease;
        }

        l =
          (distances[i] - distances.min) / distances.max ||
          0;
        return (
          _round(
    ...
    _replaceRandom = function _replaceRandom(value) {
      ...
      while (~(i = value.indexOf("random(", prev))) {
        ...
        nums = value
          .substr(i + 7, end - i - 7)
          .match(
            isArray ? _delimitedValueExp : _strictNumExp
          );
        s +=
    ...
    _interrupt = function _interrupt(animation) {
      ...
      animation.scrollTrigger &&
        animation.scrollTrigger.kill(false);
    ...
    _createPlugin = function _createPlugin(config) {
      config =
        (!config.name && config["default"]) || config;
    ...
    _quotesExp = /["']/g,
    _parseObjectInString = function _parseObjectInString(
      value
    ) {
      var obj = {},
        split = value
          .substr(1, value.length - 3)
          .split(":"),
        key = split[0],
        i = 1,
        l = split.length,
      ...
      for (; i < l; i++) {
        val = split[i];
        index =
          i !== l - 1 ? val.lastIndexOf(",") : val.length;
    ...
    _valueInParentheses = function _valueInParentheses(
      value
    ) {
    ...
    _configEaseFromString = function _configEaseFromString(
      name
    ) {
    ...
    _easeInOutFromOut = function _easeInOutFromOut(
      easeOut
    ) {
      return function (p) {
        return p < 0.5
  ...
  var Animation = (function () {
    ...
    _proto.duration = function duration(value) {
      return arguments.length
        ? this.totalDuration(
            this._repeat > 0
              ? value +
                  (value + this._rDelay) * this._repeat
              : value
          )
    ...
    _proto.iteration = function iteration(
      value,
      suppressEvents
    ) {
    ...
    _proto.globalTime = function globalTime(rawTime) {
      var animation = this,
        time = arguments.length
          ? rawTime
          : animation.rawTime();

      while (animation) {
        time =
          animation._start + time / (animation._ts || 1);
  ...
  var Timeline = (function (_Animation) {
    ...
    _proto2.set = function set(targets, vars, position) {
      vars.duration = 0;
      vars.parent = this;
      _inheritDefaults(vars).repeatDelay ||
        (vars.repeat = 0);
    ...
    _proto2.previousLabel = function previousLabel(
      beforeTime
    ) {
  ...
  var _addComplexStringPropTween = function _addComplexStringPropTween(
      ...
      while ((result = _complexStringNumExp.exec(end))) {
        ...
        if (endNum !== startNums[matchIndex++]) {
          startNum =
            parseFloat(startNums[matchIndex - 1]) || 0;
          pt._pt = {
            _next: pt._pt,
    ...
    _checkPlugin = function _checkPlugin(
      ...
      if (
        ...
        if (tween !== _quickTween) {
          ptLookup =
            tween._ptLookup[tween._targets.indexOf(target)];
    ...
    _initTween = function _initTween(tween, time) {
      ...
      if (!tl) {
        ...
        for (i = 0; i < targets.length; i++) {
          target = targets[i];
          gsData =
            target._gsap || _harness(targets)[i]._gsap;
      ...
      tween._initted =
        (!tween._op || tween._pt) && !overwritten;
    },
  ...
  var Tween = (function (_Animation2) {
    ...
    function Tween(targets, vars, time, skipInherit) {
      ...
      if (
        ...
        duration ||
          _this3.duration((duration = tl.duration()));
      } else {
        _this3.timeline = 0;
      }
  ...
  var _setterPlain = function _setterPlain(
    ...
    _setterFunc = function _setterFunc(
      target,
      property,
      value
    ) {
    ...
    _renderPropTweens = function _renderPropTweens(
      ratio,
      data
    ) {
      var pt = data._pt;
    ...
    _killPropTweensOf = function _killPropTweensOf(
      property
    ) {
      var pt = this._pt,
  ...
  var PropTween = (function () {
    ...
    _proto4.modifier = function modifier(
      func,
      tween,
      target
    ) {
  ...
  var _gsap = {
    ...
    quickSetter: function quickSetter(
      target,
      property,
      unit
    ) {
    ...
    isTweening: function isTweening(targets) {
      return (
        _globalTimeline.getTweensOf(targets, true).length >
        0
      );
    },
    ...
    exportRoot: function exportRoot(
      ...
      var tl = new Timeline(vars),
        child,
        next;
      tl.smoothChildTiming = _isNotFalse(
        vars.smoothChildTiming
      );
    ...
    core: {
      ...
      suppressOverwrites: function suppressOverwrites(
        value
      ) {
  ...
  var _getPluginPropTween = function _getPluginPropTween(
    ...
    _addModifiers = function _addModifiers(
      tween,
      modifiers
    ) {
  ...
  var _win$1,
    ...
    _removeProperty = function _removeProperty(
      target,
      property
    ) {
      if (property) {
      ...
      if (endValues.length) {
        while ((result = _numWithUnitExp.exec(end))) {
          ...
          if (
            ...
            index =
              _numWithUnitExp.lastIndex - endUnit.length;
    ...
    _renderClearProps = function _renderClearProps(
      ratio,
      data
    ) {
      if (
        data.tween &&
        data.tween._time === data.tween._dur
      ) {
        var target = data.t,
          style = target.style,
          props = data.u,
    ...
    _getMatrix = function _getMatrix(target, force2D) {
      ...
      if (cache.svg && target.getAttribute("transform")) {
        temp = target.transform.baseVal.consolidate()
          .matrix;
        matrix = [
          temp.a,
          temp.b,
    ...
    _parseTransform = function _parseTransform(
      target,
      uncache
    ) {
    ...
    _renderCSSTransforms = function _renderCSSTransforms(
      ...
      if (rotation !== _zeroDeg) {
        transforms +=
          "rotate(" + rotation + _endParenthesis;
      }

      if (rotationY !== _zeroDeg) {
        transforms +=
          "rotateY(" + rotationY + _endParenthesis;
      }

      if (rotationX !== _zeroDeg) {
        transforms +=
          "rotateX(" + rotationX + _endParenthesis;
      }
    ...
    _addRawTransformPTs = function _addRawTransformPTs(
      ...
      if (startCache.svg) {
        ...
        startValue = getComputedStyle(target)[
          _transformProp
        ];
        style[_transformProp] = transforms;
        endCache = _parseTransform(target, 1);
    ...
    function (name, index) {
      ...
      _specialProps[
        ...
        if (arguments.length < 4) {
          ...
          vars = a.join(" ");
          return vars.split(a[0]).length === 5
            ? a[0]
            : vars;
        }
  ...
  var CSSPlugin = {
        ...
        if (specialProp) {
          ...
          if (isTransformRelated) {
            ...
            if (p === "scale") {
              ...
              endValue = _convertKeywordsToPercentages(
                endValue
              );
    ...
    getSetter: function getSetter(
      ...
      return property in _transformProps &&
        ...
        : target.style &&
          !_isUndefined(target.style[property])
        ? _setterCSSStyle
        : ~property.indexOf("-")
        ? _setterCSSProp
